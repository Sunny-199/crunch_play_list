...
def checkout_session(request):
        if request.method=="POST" and 'sname' in request.POST:
            ...
            card_expiry=request.POST['cardmonth']+'/'+request.POST['cardyear']
            ...
            obj=Subscription.objects.filter(id=planId).values()
            planData = obj[0]
            request.session['price'] = planData['price']
            request.session['name'] = planData['name']
            data={
                'x_login':"visanetgt_qpay ",
                'x_private_key':"88888888888",
                "x_api_secret": "99999999999",
                "x_product_id": planId,
                "x_audit_number": invoice,
                "x_fp_sequence": invoice,
                "x_fp_timestamp": timestamp,
                "x_invoice_num": invoice,
                "x_currency_code": "GTQ",
                "x_amount": planData['price'],
                "x_line_item":planData['name'],
                "x_freight": "",
                ...
                "x_type":"AUTH_ONLY",
                "x_method": "CC",
                "http_origin": "http://playlistcrunch.com",
                "visaencuotas": 0
            }
            print(data,"Data")
            req = requests.post(base_api,json=data)
            print(req,"request")
            req.raise_for_status()
            data = req.json()
            request.session['transactionData'] = data
            print(data)
            return JsonResponse(data)
        else:
            obj=Subscription.objects.all()
        return render(request,"crunch/with_login/checkout.html",{'obj':obj})

def success(request):
    price = request.session['price'] 
    name= request.session['name']
    data= request.session['transactionData']
    val= request.session['transactionData']['idTransaction']
    if(name=='Monthly'):
        subscription_end = datetime.today() + relativedelta(months=+1)
    else:
        subscription_end = datetime.today() + relativedelta(months=+12)
    current_user=request.user 
    cd=datetime.today()
    obj=userProfile.objects.filter(userFK=current_user.id).update(subscribed="True",subscription_type=name,price=price, subscription_start =cd,payment_status=True,paymentDate=cd,currency="GTQ",order_ID=val, recurring_billing=True,subscription_expiry=subscription_end)
    return render(request,"crunch/with_login/success.html",{'price':price,'name':name, 'data': data,'current_user':current_user})
...
def home(request):
    ...
    obj=Subscription.objects.all()
    return render(request,'crunch/without_login/index.html',{'bannerData':bannerData,'people':people,'obj':obj})
    ...
    if request.user.is_authenticated and not request.user.is_superuser:
        if request.method == 'GET':
            ...
            if(userID.payment_status and userID.subscribed):
                ...
                return render(request,'crunch/with_login/price_screen.html',{'obj':obj,"username":str(request.user.first_name.split(" ")[0])})
    else:
        return redirect("home")
